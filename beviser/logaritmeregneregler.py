...
class LogRegler(MovingCameraScene, Slide if slides else None):
    def construct(self):
        defs = self.definitioner(banimationer=True)
        ...
        self.play(
            LaggedStart(
                *[Uncreate(m) for m in [recs]],
                *[Unwrite(m) for m in [log_defs, pow_rules, def_titles]],
                lag_ratio=0.1
            ),
            run_time=2.5
        )
    ...
    def definitioner(self, banimationer=True):
        ...
        self.remove(log_defs, pow_rules, recs, def_titles)
    ...
    def regneregel_1(self, defs):
        ...
        self.play(
            ...
            # *[p2p_anim_copy(steps[0], steps[1], tex.tex_string) for tex in steps[1]],
            # p2p_anim_copy(steps[0], steps[1], tex1=r"\cdot", tex2="+"),
            run_time=2
        )
        self.slide_pause()
    ...
    def regneregel_3(self, defs):
        ...
        regneregel = MathTex(
            ...
            r"\log(", r"a", r"^{b}", ")", "=", "b", r"\cdot", r"\log(", "a", ")"
        ).set_color_by_tex_to_color_map(cmap).set_z_index(3)
        srec = get_background_rect(regneregel, stroke_colour=color_gradient([cmap["a"], cmap["b"]], 3))
        self.play(
            LaggedStart(
        ...
        og_term = MathTex(r"\log(", "a", "^{b}", ")").set_color_by_tex_to_color_map(cmap)

        steps = VGroup(
            MathTex(
                r"=", r"\log", r"\left(", r"\left(", "10", r"^{\log(", "a", ")}",
                r"\right)", "^b", r"\right)"
            ).set_color_by_tex_to_color_map(cmap),
            MathTex(
                r"=", r"\log", r"\left(", "10", r"^{\log(", "a", r") \cdot", "b", "}", r"\right)"
            ).set_color_by_tex_to_color_map(cmap),
            MathTex(
                r"=", r"b", r"\cdot", r"\log(", "a", ")"
            ).set_color_by_tex_to_color_map(cmap),
        ).arrange(DOWN, aligned_edge=LEFT).to_edge(RIGHT)
        og_term.next_to(steps[0], LEFT)
